...
py rag_offline\prepare_triplets.py `
  ...
  --output var\data\rag\triplets.smoke.jsonl
...
py rag_offline\eval_retrieval.py `
  ...
  --rag-mode keyword `
  --top-k 3 `
  --out var\reports\rag\eval_report.keyword.json
...
py rag_offline\tune_rag_params.py `
  ...
  --rag-mode keyword `
  --top-k 3 `
  --out var\reports\rag\tune_report.keyword.json
if ($LASTEXITCODE -ne 0) { throw "tune_rag_params failed" }

py rag_offline\export_env.py `
  --tune-report var\reports\rag\tune_report.keyword.json `
  --output var\reports\rag\best_rag.keyword.env
