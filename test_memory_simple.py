...
print("=" * 60)
print("记忆系统测试 - 丰川祥子样本个体 (独立版本)")
print("=" * 60)

# ========== 工具函数 ==========

def memory_weight(now_ts, last_seen, seen_count, strength, half_life_days=14.0, alpha=0.08):
    """计算记忆权重（遗忘 + 强化）"""
...
def recency_score(last_seen, now_ts=None, half_life_days=7.0):
    """计算时间新鲜度分数"""
    if now_ts is None:
...
print(f"✓ 角色创建完成：{sakiko_config['name']}")
print(f"  人格特质：开放性={sakiko_config['openness']:.2f}, 外向性={sakiko_config['extraversion']:.2f}")
print(f"  记忆配置：半衰期={sakiko_config['half_life_days']}天，搭话间隔={sakiko_config['idle_threshold_sec']}秒")
...
for text, mem_type, emotion, importance in test_memories:
    if mem_type == "episodic":
        cur.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, (sakiko_config['id'], text, ts, ts, emotion, importance))
    else:
        cur.execute("""
            INSERT INTO semantic_memories
            (character_id, text, created_at, last_seen, category)
            VALUES (?, ?, ?, ?, ?)
        """, (sakiko_config['id'], text, ts, ts, mem_type))
...
# 统计
cur.execute("SELECT COUNT(*) FROM episodic_memories WHERE character_id=?", (sakiko_config['id'],))
episodic_count = cur.fetchone()[0]
cur.execute("SELECT COUNT(*) FROM semantic_memories WHERE character_id=?", (sakiko_config['id'],))
...
for query, keywords in test_queries:
    ...
    for row in cur.execute(
        ...
        if any(kw in row[1] for kw in keywords):
            ...
            rec = recency_score(row[3], ts, half_life_days=7.0)
            w = memory_weight(ts, row[3], 1, 0.7, half_life_days=sakiko_config['half_life_days'])
            
            final_score = sakiko_config['semantic_weight'] * score + 0.15 * rec + sakiko_config['emotion_weight'] * w
...
✓ 高开放性 (0.75) → 语义权重 {sakiko_config['semantic_weight']:.2f}
  效果：更容易联想 distant memories，检索范围更广

✓ 低外向性 (0.35) → 搭话间隔 {sakiko_config['idle_threshold_sec']}秒，最多{sakiko_config['max_nudges']}次
  效果：不会频繁主动搭话，符合内向性格

✓ 低宜人性 (0.45) → 情绪权重 {sakiko_config['emotion_weight']:.2f}
  效果：不太容易被情绪左右，更理性

✓ 高神经质 (0.70) → 情绪偏向检索：{sakiko_config['emotion_bias_retrieval']}
...
w = memory_weight(now, last_seen, seen_count, strength, half_life_days=sakiko_config['half_life_days'])
print(f"  7 天前的记忆（见过{seen_count}次）：权重={w:.3f}")

last_seen = ts - (14 * 86400)  # 14 天前
w2 = memory_weight(now, last_seen, seen_count, strength, half_life_days=sakiko_config['half_life_days'])
print(f"  14 天前的记忆（见过{seen_count}次）：权重={w2:.3f}")

last_seen = ts - (14 * 86400)  # 14 天前
seen_count = 10  # 见过很多次
w3 = memory_weight(now, last_seen, seen_count, strength, half_life_days=sakiko_config['half_life_days'])
...
# 导出记忆
print("\n记忆导出:")
for row in cur.execute("SELECT id, text, emotion, importance FROM episodic_memories WHERE character_id=?", (sakiko_config['id'],)):
    print(f"  [情景] {row[1]} (情绪={row[2]}, 重要性={row[3]})")

for row in cur.execute("SELECT id, text, category FROM semantic_memories WHERE character_id=?", (sakiko_config['id'],)):
